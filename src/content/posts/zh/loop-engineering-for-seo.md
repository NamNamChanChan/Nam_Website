...
description: "用 SEO 實例講解 Loop Engineering：找出一個機會、做一項有邊界的改動、驗證、等待證據，再決定重複還是停止。"
...
真正經過工程設計的 SEO Loop 幾乎相反。它先讀取你自己網站的證據，只選擇**一個現有頁面**，提出**一項有邊界的改善**，通過技術與編輯檢查，停下來讓人審批，記錄發生過甚麼，然後等到有足夠數據才判斷改動是否有效。
...
在 28 日 finalised data 裡，頁面從包括「private AI deployment checklist」的 query cluster 得到 12,400 impressions 和 190 clicks，即 1.53% click-through rate，average position 是 7.8。
...
| 證據                                      | Loop 決定                         |
| ----------------------------------------- | --------------------------------- |
| Clicks 與 CTR 改善，而 query 仍然相關     | 記錄為有用改動，然後停止          |
...
1. 用 final data 拉一份 28 日 Search Console 比較。
2. 要求 Loop 最多提出五個 candidates，並列出每項證據。
3. 自己選擇一個現有頁面。
4. 只容許一項有邊界的改動，並要求可讀的 diff。
5. 跑技術與 editorial checks。
6. 只有人手批准後才發佈。
7. 將 baseline 與量度日期寫入 state。
8. 手動完成六個 cycle，再考慮自動化 discovery。

到時你會知道漏了哪些規則、哪些 signal 太嘈，以及 Loop 節省的判斷是否多過它消耗的判斷。

## 真正的重點

SEO 不需要另一部可以生成一千篇文章的機器。它需要一個 feedback system：找出一個值得處理的機會，小心改善，亦懂得在證據說「等」或「停」時承認。

這就是 Loop Engineering 帶來的東西：不是無限自主，而是有結構地重複。

真正有用的單位不是文章，而是**有證據支持的決定**。
