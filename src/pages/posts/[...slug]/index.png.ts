...
export async function getStaticPaths() {
  ...
  // EN-only: the satori font has no CJK glyphs, so zh posts use the
  // site-wide default OG image instead (see PostDetail.astro).
  const posts = await getCollection("posts").then(p =>
    filterByLang(p, "en").filter(({ data }) => !data.draft && !data.ogImage)
  );
